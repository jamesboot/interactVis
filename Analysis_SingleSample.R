...
# Change working directory
setwd(
  '/Users/jamesboot/Documents/9.Genome Centre Files/GC-TM-10271/interactVis_trial/'
)
base.dir <-
  '/Users/jamesboot/Documents/9.Genome Centre Files/GC-TM-10271/interactVis_trial/SingleSampleTrial/'

# Create list of all sample directories
dirs <-
  list.dirs(path = '/Users/jamesboot/Documents/9.Genome Centre Files/GC-TM-10271/4 Runs Combined/spatial',
            recursive = F)

# Create list of all sample .h5 files
files <-
  list.files(
    path = '/Users/jamesboot/Documents/9.Genome Centre Files/GC-TM-10271/4 Runs Combined/spatial',
    pattern = '*.h5',
    recursive = T,
    full.names = T
  )
...
#### PART 2: analyse 1 sample and perform interaction analysis ----
...
# Find neighbors
neighbours <-
  findNeighbours(file.path(dirs[1], 'spatial/tissue_positions.csv'))
...
dir.create(path = file.path(base.dir, samples[1]))

# Save interaction object
saveRDS(interactions,
        file = paste0(base.dir, '/', samples[1], '/', samples[1], '_interactions.rds'))

# Save complex summary as csv
write.csv(
  interactions$Complex_Summary,
  file = paste0(base.dir, '/', samples[1], '/', samples[1], '_complexSummary.csv')
)
...
partekMetaFilt <-
  partekMeta[partekMeta$Sample.name == samples[1],]
...
ggsave(
  plot = plt,
  filename = paste0(base.dir, '/', samples[1], '/', samples[1], '_cluster.tiff'),
  height = 5,
  width = 5,
  dpi = 300,
  units = 'in'
...
write.csv(interactionSum,
          file = paste0(base.dir, '/', samples[1], '/', samples[1], '_nInt_cluster.csv'))
...
# Plot heatmap
tiff(
  filename = paste0(base.dir, '/', samples[1], '/', samples[1], '_htmap.tiff'),
  height = 5,
  width = 5,
  res = 300,
  units = 'in'
...
nInt_files <-
  list.files(
    path = '/Users/jamesboot/Documents/9.Genome Centre Files/GC-TM-10271/interactVis_trial/SingleSampleTrial/',
    recursive = T,
...
# Chord plot
tiff(
  filename = paste0(base.dir, '/', samples[1], '/', samples[1], '_chord.tiff'),
  height = 4,
  width = 4,
  units = 'in',
  res = 300
...
chordDiagram(nInt_objs[[1]], transparency = 0.5, grid.col = grid.col)
dev.off()
  
#### PART 4: Differential clusters analysis ----

# Perform differential interaction
...
write.csv(
  diffInt2v10$ReceiverResults,
  file = paste0(base.dir, '/', samples[1], '/', samples[1], '_RECEIVER_glm.csv')
)

# Write SENDER results to file
write.csv(
  diffInt2v10$SenderResults,
  file = paste0(base.dir, '/', samples[1], '/', samples[1], '_SENDER_glm.csv')
)
